...
- (void)configToolView {
    ...
    self.toolView.backgroundColor = RGBCOLOR(245, 245, 245);
    ...
    CGFloat lblWidth = RSScreenWidth/5 ;
    UIView * xView;
    for (int i =0; i <= 4; i ++) {
        ...
        [lbl mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            {
                make.left.equalTo(xView.mas_right).offset(0);
            }
    ...
    [saveBtn setTitleColor:RGBCOLOR(57, 132, 246) forState:UIControlStateNormal];
    ...
    [cancelBtn setTitleColor:RGBCOLOR(57, 132, 246) forState:UIControlStateNormal];
...
- (void)configPickerView {
    ...
    [self.pickerView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.left.width.equalTo(self);
        make.height.equalTo(self.mas_height).offset(-84);
    }];
}
...
- (void)setDate:(NSString *)date {
    ...
    self.timeArr = [self transformArrayFromString:date];
}

#pragma mark -- 获取系统时间
- (NSString *)getSystemYearStr
{
    NSDate *date = [NSDate date];
    NSDateFormatter* dateFormatter = [[NSDateFormatter alloc]init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm"];
    NSString * dateStr = [dateFormatter stringFromDate:date];
    NSArray * tempArr = [self transformArrayFromString:dateStr];
    NSString * yearStr = tempArr[0];;
    return yearStr;
}
...
/**
 ...
 @return 日期数组
 */
- (NSMutableArray *)transformArrayFromString:(NSString *)dateStr
{
    NSString *newDate = [[dateStr stringByReplacingOccurrencesOfString:@"-" withString:@" "] stringByReplacingOccurrencesOfString:@":" withString:@" "];
...
- (void)show {
    ...
    if (_selectedDate) {
        NSArray * dateArray = [self transformArrayFromString:_selectedDate];
        self.year = dateArray[0];
        self.month = [NSString stringWithFormat:@"%ld月", [dateArray[1] integerValue]];
        self.day = [NSString stringWithFormat:@"%ld日", [dateArray[2] integerValue]];
        self.hour = [NSString stringWithFormat:@"%ld时", [dateArray[3] integerValue]];
        self.minute = self.minuteInterval == 1 ? [NSString stringWithFormat:@"%ld分", [dateArray[4] integerValue]] : self.minuteArr[self.minuteArr.count / 2];
    }
    else
    {
...
//                    [pickerView selectRow:[self.dataArray[component] indexOfObject:self.timeArr[component]] inComponent:component animated:YES];
//                    NSInteger tempRow = [self.dataArray[component] indexOfObject:self.timeArr[component]];
//                    self.minute = self.minuteArr[tempRow];
//
//                } else
//                {
//                    // 如果选择分大于当前分，就直接赋值
